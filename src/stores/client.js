...
export const useClientsStore = defineStore('clients', async () => {
  // Déclaration de la liste des routes avec `ref`
  const list = ref ([])
  const loginStore = await LoginStore()
  const urlBase = "https://a14vinotrip-fab8apb7c9aeergn.eastus-01.azurewebsites.net/api/"
  

  watch(() => loginStore.isAuthenticated, async (connected) => {
    if (connected) {
      await GetClients();
      console.log(list)
    }
  ...
  return { list,ModifieClient,AddClient,DelClient };
});
